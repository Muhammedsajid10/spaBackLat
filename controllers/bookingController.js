...
const bookingBenefitResolver = require('../services/bookingBenefitResolver');
// const { apiUtils } = require('../utils/apiUtils');
...
const buildAdminBookingPreview = async ({
  incomingServices,
  appointmentDate,
  ...
  if (!incomingServices || !Array.isArray(incomingServices) || incomingServices.length === 0) {
    throw Object.assign(new Error('At least one service is required'), { statusCode: 400 });
  }

  if (!appointmentDate) {
    throw Object.assign(new Error('appointmentDate is required'), { statusCode: 400 });
  }

  const apptDateObj = new Date(appointmentDate);
  if (Number.isNaN(apptDateObj.getTime())) {
    throw Object.assign(new Error('Invalid appointmentDate'), { statusCode: 400 });
  }

  // 1. Data Fetching
  const requestedServiceIds = [...new Set(incomingServices.map(raw => String(raw.service || raw._id || ''))).values()]
    .filter(Boolean);
  const requestedEmployeeIds = [...new Set(incomingServices.map(raw => String(raw.employee || raw.employeeId || '')).values())]
    .filter(Boolean);

  const [serviceDocs, employeeDocs] = await Promise.all([
    Service.find({ _id: { $in: requestedServiceIds } }).select('name price duration'),
    Employee.find({ _id: { $in: requestedEmployeeIds }, isActive: true })
      .populate('user', 'firstName lastName')
      .select('user position employeeId isActive workSchedule')
  ]);

  const serviceMap = new Map(serviceDocs.map(s => [String(s._id), s]));
  const employeeMap = new Map(employeeDocs.map(e => [String(e._id), e]));

  // 2. Service Transformation & Validation
  const transformedServices = [];
  for (const raw of incomingServices) {
    const serviceId = String(raw.service || raw._id || '');
    const employeeId = String(raw.employee || raw.employeeId || '');
    const serviceDoc = serviceMap.get(serviceId);
    const employeeDoc = employeeMap.get(employeeId);

    if (!serviceDoc) throw Object.assign(new Error(`Service not found: ${serviceId}`), { statusCode: 404 });
    if (!employeeDoc) throw Object.assign(new Error(`Professional not found for ${serviceDoc.name}`), { statusCode: 404 });

    const startTime = new Date(raw.startTime);
    if (Number.isNaN(startTime.getTime())) {
      throw Object.assign(new Error(`Invalid start time for ${serviceDoc.name}`), { statusCode: 400 });
    }

    // Check Cutoff
    const cutoffCheck = bookingAvailabilityService.checkEndTimeCutoff(startTime, serviceDoc.duration);
    if (cutoffCheck.exceeded) {
      throw Object.assign(
        new Error(`Service "${serviceDoc.name}" ends at ${cutoffCheck.endTimeStr}, which is after 23:30 cutoff.`),
        { statusCode: 400 }
      );
    }

    const { basePrice, finalPrice, discount, isCustom } = bookingPricingService.getServicePrice(serviceDoc, raw);
    const employeeName = `${employeeDoc.user?.firstName || ''} ${employeeDoc.user?.lastName || ''}`.trim();
    ...
    transformedServices.push({
      ...
      basePrice,
      finalPrice,
      customPrice: isCustom ? finalPrice : null,
  ...
  const availability = await bookingAvailabilityService.validateAvailability(
    transformedServices.map(s => ({ ...s, employeeId: String(s.employeeId) })),
    apptDateObj
  );
  if (!availability.valid) {
    throw Object.assign(new Error(availability.errors[0].message), { statusCode: 409 });
  }

  // 4. Benefit Resolution (Comprehensive)
  const normalizedPaymentDetails = { ...(incomingPaymentDetails || {}) };
  let membershipSummary = null;
  let membershipDiscount = 0;
  
  const clientToResolve = incomingPaymentDetails?.clientId || incomingPaymentDetails?.client?._id;
  const allBenefits = await bookingBenefitResolver.resolveBenefits(clientToResolve, transformedServices);
  const eligibleMemberships = allBenefits.memberships;

  const requestedMembershipId = normalizedPaymentDetails?.adminMembership?.membershipId;
  if (requestedMembershipId) {
    const match = eligibleMemberships.find(m => String(m.membershipId) === String(requestedMembershipId));

    if (match) {
      const matchedService = transformedServices.find(s => match.matchedServiceIds.includes(String(s.serviceId)));
      if (matchedService) {
        membershipDiscount = matchedService.finalPrice;
        membershipSummary = {
          ...
          remainingSessionsBefore: match.remainingSessions
        };
        normalizedPaymentDetails.adminMembership = {
          ...membershipSummary,
  ...
  // 5. Gift Card
  let giftCardSummary = null;
  let giftCardAmount = 0;
  const gcCode = normalizedPaymentDetails?.giftCard?.code || normalizedPaymentDetails?.giftCardCode;
  if (gcCode) {
    const gc = await bookingBenefitResolver.resolveGiftCard(gcCode);
    if (gc) {
      const currentSubtotal = transformedServices.reduce((sum, s) => sum + s.finalPrice, 0);
      const remainingAfterMem = currentSubtotal - (customDiscount || 0) - membershipDiscount;
      giftCardAmount = Math.min(remainingAfterMem, gc.balance);
      giftCardSummary = {
        giftCardId: gc.id,
        code: gc.code,
        redeemAmount: giftCardAmount,
        remainingValueBefore: gc.balance
      };
      normalizedPaymentDetails.giftCard = { ...giftCardSummary };
    }
  }

  // 6. Final Pricing
  const pricing = bookingPricingService.calculateFinalPricing(
    transformedServices.map(s => ({ ...s, price: s.finalPrice })),
    { customDiscount, membershipDiscount, giftCardAmount }
  );
  ...
  return {
    ...
    benefits: {
      ...
      eligibleMemberships // NEW: pass the whole list for the UI to show
    },
    normalizedPaymentDetails,
    previewServices: transformedServices.map(s => ({
      ...s,
      membershipApplied: membershipSummary?.matchedServiceId && String(membershipSummary.matchedServiceId) === String(s.serviceId),
      giftCardAppliedAmount: 0 // Logic for splitting GC across services could be added later
    }))
  };
};

const previewAdminBooking = async (req, res) => {
  try {
    const preview = await buildAdminBookingPreview({
      incomingServices: req.body.services,
      appointmentDate: req.body.appointmentDate,
      incomingPaymentDetails: req.body.paymentDetails,
      customDiscount: req.body.customDiscount
    });

    res.json({
      success: true,
      data: {
        services: preview.previewServices,
        pricing: preview.pricing,
        benefits: preview.benefits,
        normalizedPaymentDetails: preview.normalizedPaymentDetails,
        warnings: [],
        errors: []
      }
    });
    ...
    res.status(statusCode).json({
      success: false,
      message: error.message || 'Failed to preview booking'
    });
  }
};
...
const createBooking = async (req, res) => {
  try {
    ...
    if (!clientUser && clientData) {
      ...
      const firstName = clientData.firstName || parts[0] || 'Walk-in';
      const lastName = clientData.lastName || parts.slice(1).join(' ') || 'Customer';
      
      const tempEmail = clientData.email || `temp_${Date.now()}_${Math.random().toString(36).substring(2, 7)}@allora.internal`;
      const cleanPhone = clientData.phone ? clientData.phone.replace(/[^\d+]/g, '') : null;
      
      clientUser = new User({
        firstName, lastName, email: tempEmail, phone: cleanPhone,
        password: Math.random().toString(36).substring(2, 10),
        role: 'client', isEmailVerified: false
      });
      await clientUser.save();
    } else if (!clientUser) {
      clientUser = req.user;
    ...
    if (!clientUser?._id) {
      return res.status(400).json({ success: false, message: 'Unable to determine client information.' });
    }

    // 3. SERVICE, AVAILABILITY, AND PAYMENT PREVIEW VALIDATION
    const preview = await buildAdminBookingPreview({
      incomingServices,
      appointmentDate,
      incomingPaymentDetails,
    ...
    const transformedServices = preview.transformedServices;
    const calculatedTotalAmount = preview.pricing.subtotal;
    const totalDuration = preview.totalDuration;
    const finalAmount = preview.pricing.finalAmount;
    const normalizedPaymentDetails = preview.normalizedPaymentDetails;
    ...
    const newBooking = new Booking({
      ...
      internalNotes: internalNotes || 'Admin booking',
      specialRequests: specialRequests || [],
      finalAmount,
      customDiscount: preview.pricing.manualDiscount || 0,
      paymentDetails: normalizedPaymentDetails
    });

    // Booking number is auto-generated in pre-save hook of Booking.js
    await newBooking.save();

    // 6. PROCESS DEDUCTIONS (Membership / Gift Card)
    // Membership session deduction
    const membershipId = newBooking.paymentDetails?.membershipId || newBooking.paymentDetails?.adminMembership?.membershipId;
    if (membershipId) {
      try {
        const membership = await Membership.findById(membershipId);
        if (membership && membership.usedSessions < membership.numberOfSessions) {
          await membership.useSession();
        }
      } catch (err) {
        console.error('Membership deduction failed:', err);
      }
    }

    // Gift card value deduction
    if (newBooking.paymentDetails?.giftCard?.redeemAmount) {
      try {
        const gcId = newBooking.paymentDetails.giftCard.giftCardId;
        const gc = await GiftCard.findById(gcId);
        if (gc) {
          await gc.useGiftCard(newBooking.paymentDetails.giftCard.redeemAmount, clientUser._id, newBooking._id, 'Booking Payment');
        }
      } catch (err) {
        console.error('Gift card deduction failed:', err);
      }
    }
    ...
    try {
      const Payment = require('../models/Payment');
      const mappedMethod = (paymentMethod === 'upi' || paymentMethod === 'online') ? 'digital_wallet' : (paymentMethod || 'cash');
      const gateway = (paymentMethod === 'upi' || paymentMethod === 'online') ? 'razorpay' : 'stripe';
      ...
      const paymentRecord = new Payment({
        ...
        gatewayTransactionId: `TXN-${newBooking._id}-${Date.now()}`
      });
      await paymentRecord.save();
    } catch (err) {
    ...
    if (populated.client?.email) {
      try {
        const emailService = new EmailService();
        await emailService.sendBookingConfirmation(populated, { amount: finalAmount * 100 });
      } catch (err) {
...
// Optimized Calendar View using Aggregation Pipeline
const getCalendarView = async (req, res) => {
  try {
    ...
    const calendarData = await Booking.aggregate([
      {
        $match: {
      ...
      { $unwind: { path: "$employeeUserDetails", preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          from: "services",
      ...
      { $unwind: { path: "$serviceDetails", preserveNullAndEmptyArrays: true } },
      {
        $project: {
          ...
          bookingNumber: 1,
          appointmentDate: 1,
          status: { $ifNull: ["$services.status", "$status"] },
          client: {
            $concat: [
          ...
          service: { $ifNull: ["$serviceDetails.name", "Service"] },
          employee: {
            $concat: [
              { $ifNull: ["$employeeUserDetails.firstName", ""] }, 
              " ", 
              { $ifNull: ["$employeeUserDetails.lastName", ""] }
            ]
          },
          employeeId: "$employeeData._id",
          duration: { $ifNull: ["$services.duration", 30] },
          ...
          serviceEntryId: "$services._id",
          notes: "$services.notes"
        }
      }
    ]);

    // Group into the exact slotKey structure expected by frontend
    const groupedData = calendarData.reduce((acc, curr) => {
      const empId = curr.employeeId?.toString() || 'unassigned';
      if (!acc[empId]) acc[empId] = {};
      
      const startDT = new Date(curr.startTime);
    ...
    console.error('Error in getCalendarView aggregation:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch optimized calendar view',
      error: error.message
    });
  }
};
