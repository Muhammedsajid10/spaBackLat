...
const deleteService = catchAsync(async (req, res, next) => {
  ...
  // No active bookings - safe to remove permanently
  try {
    ...
    console.error('Error deleting service:', delErr);
    // As a fallback, attempt a soft-delete so admin can still hide it
    try {
      service.isActive = false;
      await service.save();
      return res.status(200).json({
        success: true,
        message: 'Service deactivated (soft-deleted) after delete error',
        data: null
      });
    } catch (softErr) {
      console.error('Error soft-deleting service after delete failure:', softErr);
